...
static inline int      KL_ISEMPTY(void const * const block_hdr);
...
static inline int KL_ISEMPTY(void const * const block_hdr)
{
  ...
  size = ((KL_BLOCK_SIZE(KL_BLOCK_DEFAULT_SIZE)-KL_BLOCK_HEADER_SIZE) /
    ((KL_G_MULT(block_hdr)+1)*KL_MEMORY_ALLOCATION_ALIGNMENT));

  return size == KL_G_COUNT(block_hdr);
}
...
kl_chunk_solo(size_t const size)
{
  ...
  *KL_B2F(block, block_size) = *KL_B2H(block);
  ...
  /* Set the chunk to be returned. */
  chunk = (void*)((uintptr_t)block+KL_BLOCK_HEADER_SIZE);
