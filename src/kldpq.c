...
main()
{
  kl_dpq_t        dpq;
  kl_dpq_bucket_t * b;
  kl_dpq_node_t   * n, * n3, * n17, * n23, * n2;

  kl_dpq_init(&dpq, 4096);

  n3  = kl_dpq_new(&dpq, 3);
  n17 = kl_dpq_new(&dpq, 17);
  n23 = kl_dpq_new(&dpq, 23);
  n2  = kl_dpq_new(&dpq, 2);

  kl_dpq_inc(&dpq, n3); kl_dpq_inc(&dpq, n3);
  kl_dpq_inc(&dpq, n23); kl_dpq_inc(&dpq, n23); kl_dpq_inc(&dpq, n23);
  kl_dpq_inc(&dpq, n23); kl_dpq_inc(&dpq, n23); kl_dpq_inc(&dpq, n23);
  kl_dpq_inc(&dpq, n17); kl_dpq_inc(&dpq, n17); kl_dpq_inc(&dpq, n17);
  kl_dpq_inc(&dpq, n17);
  kl_dpq_inc(&dpq, n2);

  kl_dpq_dec(&dpq, n3);
  kl_dpq_dec(&dpq, n23); kl_dpq_dec(&dpq, n23);
  kl_dpq_dec(&dpq, n17);
  kl_dpq_dec(&dpq, n2);
  ...
  fprintf(stderr, "dpq.cap: %lu\n", dpq.cap);
  fprintf(stderr, "dpq.scap: %lu\n", dpq.scap);
  fprintf(stderr, "dpq.empty: %d\n", kl_dpq_empty(&dpq));
  fprintf(stderr, "dpq.hd->k: %u\n", (kl_dpq_peek(&dpq))->k);
  fprintf(stderr, "dpq.hd->bid: %u\n", (kl_dpq_peek(&dpq))->bid);
  fprintf(stderr, "dpq.tl->k: %u\n", dpq.tl->hd->k);
  fprintf(stderr, "dpq.tl->bid: %u\n", dpq.tl->hd->bid);
