...
/****************************************************************************/
/*! Allocate memory via anonymous mmap and zero out (done automatically by
 *  sb_malloc. */
/****************************************************************************/
extern void *
...
#if 0
extern void *
sb_realloc(void * const addr, size_t const len)
{
  int fd=-1;
  size_t ip, app_addr, npages, new_npages, msize, new_msize, psize, new_psize;
  size_t num=0, ld_pages=0, new_app_addr=(size_t)MAP_FAILED;
  char * pflags, * new_pflags=NULL;
  struct sb_alloc * sb_alloc;
  ...
  if (NULL == (sb_alloc=sb_internal_find((size_t)addr))) {
    SBWARN(SBDBG_FATAL)("attempt to reallocate an unhandled memory location "
      "(%p)", addr);
    return NULL;
  }

  psize      = sb_info.pagesize;
  new_npages = 1+((len-1)/psize);
  new_msize  = new_npages*psize;
  ...
  if (new_msize < msize) { /* easy case */
    SBMADVISE(app_addr+new_msize, msize-new_msize, MADV_DONTNEED);
    sb_alloc->len    = len;
    sb_alloc->npages = new_npages;
  }
  else if (msize < new_msize) {
    /* allocate the new pflags */
    new_pflags = (char *)(*sb_info.malloc)(new_npages*sizeof(char));
    if (NULL == new_pflags) {
    ...
    if (-1 == (fd=open(sb_alloc->fname, O_RDWR, 0))) {
      SBWARN(SBDBG_DIAG)("%s", strerror(errno));
      goto CLEANUP;
    }

    /* resize file */
    if (-1 == ftruncate(fd, new_msize)) {
      SBWARN(SBDBG_DIAG)("%s", strerror(errno));
      goto CLEANUP;
    }

    /* get the new anonymous map */
    SBMMAP(new_app_addr, new_msize, PROT_WRITE, fd);
    if (new_app_addr == app_addr) {
      SBWARN(SBDBG_DIAG)("invalid address");
      goto CLEANUP;
    }
    ...
    /* copy the data */
    memcpy((void*)new_app_addr, (void*)app_addr, sb_alloc->len);

    SBMPROTECT(new_app_addr, msize, PROT_READ);
    ...
    for (ip=0; ip<npages; ip++) {
      if (SBISSET(new_pflags[ip], SBPAGE_DIRTY))
        SBMPROTECT(new_app_addr+(ip*psize), psize, PROT_READ|PROT_WRITE);
      else
        new_pflags[ip] = (pflags[ip]|SBPAGE_SYNC);
    }
    for (; ip<new_npages; ip++)
      new_pflags[ip] = SBPAGE_SYNC;

    (*sb_info.free)(pflags);

    sb_alloc->app_addr = new_app_addr;
    sb_alloc->len      = len;
    sb_alloc->pagesize = new_psize;
    sb_alloc->npages   = new_npages;
    sb_alloc->ld_pages = new_npages;
    sb_alloc->pflags   = new_pflags;
  }

  if (new_msize < msize) {
    /* need to count number of loaded pages */
    for (num=0,ip=new_msize/psize; ip<npages; ++ip) {
      if (SBISSET(pflags[ip], SBPAGE_SYNC) ||
          SBISSET(pflags[ip], SBPAGE_DIRTY)) {
        num++;
      }
      pflags[ip] &= ~(SBPAGE_SYNC|SBPAGE_DIRTY|SBPAGE_ONDISK);
    }
    sb_assert(sb_alloc->ld_pages >= num);
    sb_alloc->ld_pages -= num;
    sb_internal_acct(SBACCT_DISCHARGE, SB_TO_SYS(num, psize));
  }
  else {
    sb_internal_acct(SBACCT_READ, num);
    num = new_msize-(ld_pages*psize);
    sb_internal_acct(SBACCT_ALLOC, 1+((num-1)/sysconf(_SC_PAGESIZE)));
  }
  SB_LET_LOCK(&(sb_alloc->lock));
...
CLEANUP:
  SB_LET_LOCK(&(sb_alloc->lock));
  if (MAP_FAILED != (void*)new_app_addr)
    SBMUNMAP(new_app_addr, new_msize);
  if (NULL != new_pflags)
    (*sb_info.free)(new_pflags);
  if (-1 != fd)
    (void)close(fd);
  return NULL;
}
